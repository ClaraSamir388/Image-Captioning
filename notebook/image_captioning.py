...
Original file is located at
    https://colab.research.google.com/drive/1Cxc5ALOsJTgChCbpiZHOm7OF6LE0coXE
"""
...
"""to help in deployment"""

# -----------------------------------------
# caption_model_inference.py
# -----------------------------------------
...
# ---------------------------------------------------
# 1) Vocabulary class (NEEDED for mapping idx <-> word)
# ---------------------------------------------------
class Vocabulary:
    def __init__(self):
        self.word2idx = {}
    ...
    def build_from_mapping(self, mapping):
        self.word2idx = mapping
        self.idx2word = {v: k for k, v in mapping.items()}


# ---------------------------------------------------
# 2) MODEL DEFINITION (same as training model)
# ---------------------------------------------------
import math
...
class ImageCaptioningModel(nn.Module):
    def __init__(self, vocab_size, decoder_dim=512, nhead=8, num_layers=4,
                 vit_model_name="google/vit-base-patch16-224-in21k", dropout=0.1):
        super().__init__()

        self.vit = ViTModel.from_pretrained(vit_model_name)
        vit_hidden = self.vit.config.hidden_size

        self.enc_proj = nn.Linear(vit_hidden, decoder_dim)
        self.token_emb = nn.Embedding(vocab_size, decoder_dim)
        self.pos_enc = PositionalEncoding(decoder_dim)

        layer = nn.TransformerDecoderLayer(d_model=decoder_dim, nhead=nhead,
    ...
    def encode_image(self, images):
        ...
        enc = self.enc_proj(vit_out.last_hidden_state)
        return enc

    def decode_greedy(self, images, max_len, start_idx, end_idx):
        self.eval()
        with torch.no_grad():
            ...
            for _ in range(max_len - 1):
                ...
                mask = nn.Transformer.generate_square_subsequent_mask(tgt.size(0)).to(images.device)
...
ckpt = torch.load("caption_model_deploy.pt", map_location=device)

# Load vocabulary
vocab = Vocabulary()
vocab.build_from_mapping(ckpt["vocab"])

# Model
model = ImageCaptioningModel(vocab_size=len(vocab.word2idx))
...
def beam_search(model, images, beam_width=3, max_len=30,
    ...
    with torch.no_grad():
        memory = model.encode_image(images).permute(1, 0, 2)
        ...
        for _ in range(max_len):
            ...
            for score, seq in heap:
                ...
                emb = model.token_emb(tgt).permute(1, 0, 2)
                emb = model.pos_enc(emb)

                mask = nn.Transformer.generate_square_subsequent_mask(emb.size(0)).to(device)
                out = model.transformer_decoder(emb, memory, tgt_mask=mask)
                ...
                for i in range(beam_width):
                    ...
                    new_heap.append((next_score, seq + [next_tok]))
...
# ---------------------------------------------------
# 7) LOAD + PREPROCESS IMAGE
# ---------------------------------------------------
def load_image(path, enhance=True):
...
def generate_caption(path, use_beam=True, beam_size=5):
    img = load_image(path)
    tensor = transform(img).unsqueeze(0).to(device)

    if use_beam:
        ids = beam_search(model, tensor, beam_width=beam_size)
    else:
        ids = model.decode_greedy(
            tensor, max_len=30,
            start_idx=vocab.word2idx["<start>"],
    ...
    caption = ids_to_sentence(ids)
    return clean_caption(caption)


# ---------------------------------------------------
# 9) OPTIONAL: DISPLAY IMAGE + CAPTION
# ---------------------------------------------------
def display_caption(path):
    cap = generate_caption(path)
    img = load_image(path, enhance=False)

    plt.imshow(img)
    plt.title(cap, fontsize=12)
    plt.axis("off")
    plt.show()